...
def get_image_token(api, image):
    ...
    try:
        response    = requests.post(post_url, files={'content': image}, headers=api.booru_headers)
...
def check_similarity(api, image_token):
    ...
    try:
        response = requests.post(post_url, headers=api.booru_headers, data=metadata)
...
def upload_file(api, post, file_path):
    ...
    try:
        res = requests.post(post_url, headers=api.booru_headers, data=metadata)
        #print(res.json())
        os.remove(file_path)
    except Exception as e:
...
def delete_posts(api, start_id, finish_id):
    ...
    for id in range(start_id, finish_id + 1):
        ...
        try:
            requests.delete(post_url, headers=api.booru_headers, data=json.dumps({'version': '1'}))
        except Exception as e:
